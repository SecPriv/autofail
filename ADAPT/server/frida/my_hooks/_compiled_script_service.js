📦
443654 /source_script_service.js
276571 /source_script_service.js.map
✄
...
// source_script_service.js
send("!!! SCRIPT INJECTED. INITIALIZING... !!!");
var retryCount = 0;
var ListClz = frida_java_bridge_default.use("java.util.List");
var PairClz = frida_java_bridge_default.use("android.util.Pair");
var FillContextClz = frida_java_bridge_default.use("android.service.autofill.FillContext");
function parseHtmlInfo(htmlInfo) {
  ...
  if (rawAttrList) {
    info.attributes = {};
    var attrList = frida_java_bridge_default.cast(rawAttrList, ListClz);
    var size = attrList.size();
    for (var i = 0; i < size; i++) {
      var pairObj = attrList.get(i);
      var pair = frida_java_bridge_default.cast(pairObj, PairClz);
...
function processViewNode(viewNode) {
  ...
  var afHints = viewNode.getAutofillHints();
  if (afHints !== null) {
...
function extractFillRequestData(request) {
  ...
  try {
    ...
    var contextsList = frida_java_bridge_default.cast(rawList, ListClz);
    var size = contextsList.size();
    for (var i = 0; i < size; i++) {
      var rawCtx = contextsList.get(i);
      var ctx = frida_java_bridge_default.cast(rawCtx, FillContextClz);
      var ctxDump = {};
...
function runHookingLogic() {
  ...
  frida_java_bridge_default.perform(function() {
    var AutofillService = frida_java_bridge_default.use("android.service.autofill.AutofillService");
    var Service = frida_java_bridge_default.use("android.app.Service");
    function hookClass(className) {
      ...
      try {
        var Clazz = frida_java_bridge_default.use(className);
    ...
    frida_java_bridge_default.choose("android.service.autofill.AutofillService", {
      onMatch: function(inst) {
        try {
          hookClass(inst.getClass().getName());
        } catch (e) {
        }
      },
      onComplete: function() {
