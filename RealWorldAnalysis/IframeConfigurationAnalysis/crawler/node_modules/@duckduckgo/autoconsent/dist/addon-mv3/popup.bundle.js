...
(() => {
  ...
  // addon/utils.ts
  function extensionDefaultConfig(storedConfig = {}) {
  ...
  async function isEnabledForDomain(domain) {
  ...
  async function setIsEnabledForDomain(domain, isEnabled) {
    const parsed = parse(domain);
    const storedExceptions = await storageGet("exceptions") || [];
  ...
  async function init() {
    ...
    const currentSite = document.querySelector("span#current-site");
    ...
    const logsDetectionstepsCheckbox = document.querySelector("input#logs-detectionsteps");
    const logsWaitsCheckbox = document.querySelector("input#logs-waits");
