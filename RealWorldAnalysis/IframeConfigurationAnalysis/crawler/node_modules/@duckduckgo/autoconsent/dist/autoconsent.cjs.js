...
function encodeNullableBoolean(value) {
  if (value === true) {
    return 1;
  }
  if (value === false) {
...
function buildStrings(existingStrings, rules) {
  const usedStrings = /* @__PURE__ */ new Set();
  function collectStrings(step) {
  ...
  const strings = new Array(usedStrings.size).fill(void 0);
  ...
  usedStrings.forEach((s) => {
    if (strings.indexOf(s) === -1) {
      const nextFreeSlot = strings.indexOf(void 0);
      strings[nextFreeSlot] = s;
    }
  });
  return strings;
}
function encodeRules(rules, existingCompactRules) {
  rules.sort((a, b) => {
    const isGeneric = (r) => !r.runContext?.urlPattern;
    const isFrame = (r) => r.runContext?.frame === true;
  ...
  function encodeRuleStep(step) {
    const clonedStep = { ...step };
    if (step.comment) {
      delete clonedStep.comment;
    }
  ...
  const compactRules = rules.map((r) => {
    return [
      ...
      r.intermediate !== void 0 ? { intermediate: r.intermediate } : {}
    ];
  });
  return {
    v: 1,
