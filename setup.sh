...
if command -v uv >/dev/null 2>&1; then
    echo "[1/8] uv already installed: $(command -v uv)"
else
...
if [ -x "$VENV_DIR/bin/python" ]; then
    echo "[3/8] Virtualenv already exists, reusing it."
else
    echo "[3/8] Creating virtualenv at $VENV_DIR..."
    uv venv --python "$PYTHON_VERSION" "$VENV_DIR"
fi
echo

# 4. Dependencies
echo "[4/8] Installing dependencies (flask, frida)..."
...
if [ -x "$PLATFORM_TOOLS_DIR/adb" ]; then
    echo "[5/8] platform-tools already installed: $PLATFORM_TOOLS_DIR/adb"
else
    echo "[5/8] Installing Android platform-tools (adb)..."
...
if command -v node >/dev/null 2>&1 && [ "$(node -v 2>/dev/null | cut -d. -f1 | tr -d v)" -ge 18 ]; then
    echo "[6/8] Node.js already installed: $(command -v node) ($(node -v))"
elif [ -x "$NODE_BIN_DIR/node" ] && [ "$("$NODE_BIN_DIR/node" -v 2>/dev/null | cut -d. -f1 | tr -d v)" -ge 18 ]; then
    echo "[6/8] Node.js already installed: $NODE_BIN_DIR/node ($("$NODE_BIN_DIR/node" -v))"
else
    echo "[6/8] Installing Node.js (LTS)..."
    NODE_ARCH="$(uname -m)"
    case "$NODE_ARCH" in
...
if command -v java >/dev/null 2>&1; then
    ...
    if [ "${JAVA_MAJOR:-0}" -ge 17 ]; then
        echo "[7/8] Java $JAVA_MAJOR already on PATH."
        JAVA_USING_SYSTEM=1
    fi
fi
if [ "$JAVA_USING_SYSTEM" -eq 0 ]; then
    if [ -x "$JAVA_DIR/Contents/Home/bin/java" ]; then
        echo "[7/8] Java 17 already downloaded at $JAVA_DIR"
    else
        echo "[7/8] Installing Java 17 (Eclipse Temurin)..."
        case "$(uname -sm)" in
